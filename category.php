...
if (isset($_GET["category"])){
	$sokOrd =$_GET["category"];
	$context  = stream_context_create(array('http' => array('header' => 'Accept: application/xml')));
	$url= $urlPre."activities?categories=".$sokOrd;
	...
	if (isset($_GET["sokfras"]) or isset($_GET["age_1"]) or isset($_GET["age_2"]) ){}else {
	
	$context  = stream_context_create(array('http' => array('header' => 'Accept: application/xml')));
	$url=$urlPre."categories/";
	$contents = file_get_contents($url); 
	$results = json_decode($contents); 
	foreach ($results as $categories){
		//echo $categories->media_file->uri;
		echo "<img src='".$categories->media_file->uri."'>";
